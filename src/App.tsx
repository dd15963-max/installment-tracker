...
import { markPaymentsPaid } from './overdue-utils'
...
type AppModal = 'form' | 'detail' | 'split' | 'ending' | 'overdue' | 'add-choice' | 'import-preview' | null
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const appModals = new Set(['form', 'detail', 'split', 'ending', 'overdue', 'add-choice', 'import-preview'])
  ...
  function markSelectedPayments(keys: ReadonlySet<string>) {
    const paidAt = new Date().toISOString()
    setItems(previous => markPaymentsPaid(previous, keys, paidAt))
  }
  ...
  return <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchCancel={() => { swipeStart.current = null }} className="mx-auto min-h-screen max-w-[480px] touch-pan-y overflow-x-hidden bg-[#f8f7fb] shadow-2xl dark:bg-[#15141b] dark:text-white">
    ...
    {modal === 'overdue' && <OverdueScreen items={items} onClose={closeView} onMarkPaid={markSelectedPayments}/>}
...
function HomeView({ items, recurringExpenses, selectedMonth, monthOptions, setSelectedMonth, remaining, onOpen, onAdd, onOpenSplit, onOpenEnding, onOpenOverdue, onPay, onRemoveSamples }: { items: InstallmentItem[]; recurringExpenses: RecurringExpense[]; selectedMonth: string; monthOptions: string[]; setSelectedMonth: (value: string) => void; remaining: number; onOpen: (item: InstallmentItem) => void; onAdd: () => void; onOpenSplit: () => void; onOpenEnding: () => void; onOpenOverdue: () => void; onPay: (item: InstallmentItem, paymentId: string) => void; onRemoveSamples: () => void }) {
  ...
  return <>
    ...
    <section className="px-5">
      ...
      {(todayRows.length > 0 || allOverdueRows.length > 0) && <button type="button" onClick={allOverdueRows.length ? onOpenOverdue : undefined} disabled={!allOverdueRows.length} className="mb-3 w-full rounded-[22px] border border-amber-200 bg-amber-50 p-4 text-left disabled:cursor-default dark:border-amber-500/20 dark:bg-amber-500/10"><div className="flex items-start gap-3"><AlertTriangle className="mt-0.5 shrink-0 text-amber-600" size={20}/><div className="min-w-0 flex-1"><p className="font-extrabold text-amber-800 dark:text-amber-200">{allOverdueRows.length ? '미납 내역을 확인해 주세요' : '오늘 결제 내역이 있어요'}</p><p className="mt-1 text-xs leading-5 text-amber-700/80 dark:text-amber-200/70">오늘 결제 {todayRows.length}건 · 미납 {allOverdueRows.length}건</p><p className="text-[11px] leading-4 text-amber-700/70 dark:text-amber-200/60">날짜가 지나도 자동으로 납부 완료 처리하지 않습니다.</p></div>{allOverdueRows.length > 0 && <ChevronRight className="mt-2 shrink-0 text-amber-500" size={19}/>}</div></button>}
