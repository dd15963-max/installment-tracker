...
import type { InstallmentItem, ItemFormData, RecurringExpense, Tab } from './types'
import { calculatePaidCount, emptyForm, formatWon, formFromItem, itemFromForm, localDate, formatMonthKey, monthKey, monthPaid, monthScheduled, myRemainingAmount, myShare, nextPayment, paidAmount, progress, recurringActiveInMonth, remainingAmount, shiftMonthKey, splitShares } from './utils'
...
function normalizeItems(items: InstallmentItem[], restoreDuePayments = false): InstallmentItem[] {
  ...
  return items.map(item => {
    const payments = item.payments.map(payment => restoreDuePayments && payment.status === 'scheduled' && payment.scheduledDate < todayKey ? { ...payment, status: 'paid' as const, paidAt: `${payment.scheduledDate}T12:00:00+09:00` } : payment)
    const paidCount = payments.filter(payment => payment.status === 'paid').length
    const status = paidCount === item.installmentMonths ? 'completed' as const : 'active' as const
    return { ...item, splitPayment: item.splitPayment || false, splitParticipants: item.splitParticipants?.length >= 2 ? item.splitParticipants : [], payments, paidCount, status, ...(status === 'active' ? { completedAt: undefined } : {}) }
  })
}
...
function loadItems(): InstallmentItem[] {
  try {
    ...
    const restoreDuePayments = !localStorage.getItem(PAYMENT_RESTORE_KEY)
    const normalized = normalizeItems(stored ? JSON.parse(stored) : sampleItems(), restoreDuePayments)
    if (restoreDuePayments) localStorage.setItem(PAYMENT_RESTORE_KEY, 'done')
    return normalized
  } catch { return normalizeItems(sampleItems()) }
}

function normalizeRecurringExpenses(expenses: RecurringExpense[]): RecurringExpense[] {
  return expenses.map(expense => {
    const createdMonth = /^\d{4}-\d{2}/.test(expense.createdAt || '') ? expense.createdAt.slice(0, 7) : monthKey()
    const updatedMonth = /^\d{4}-\d{2}/.test(expense.updatedAt || '') ? expense.updatedAt.slice(0, 7) : createdMonth
    const storedPeriods = Array.isArray(expense.activePeriods)
      ? expense.activePeriods.filter(period => /^\d{4}-\d{2}$/.test(period.startMonth) && (!period.endMonth || /^\d{4}-\d{2}$/.test(period.endMonth))).map(period => ({ startMonth: period.startMonth, endMonth: period.endMonth || null }))
      : []
    const activePeriods: RecurringExpense['activePeriods'] = storedPeriods.length
      ? storedPeriods
      : expense.enabled
        ? [{ startMonth: createdMonth, endMonth: null }]
        : [{ startMonth: createdMonth, endMonth: updatedMonth }]
    activePeriods.sort((a, b) => a.startMonth.localeCompare(b.startMonth))
    return {
      ...expense,
      enabled: activePeriods.some(period => period.endMonth === null),
      splitPayment: expense.splitPayment || false,
      splitParticipants: expense.splitParticipants?.length >= 2 ? expense.splitParticipants : [],
      activePeriods,
    }
  })
}
...
function loadRecurring(): RecurringExpense[] {
  try {
    ...
    return stored ? normalizeRecurringExpenses(JSON.parse(stored)) : []
  } catch { return [] }
}
...
export default function App() {
  ...
  const monthInstallments = items.filter(item => item.payments.some(payment => payment.scheduledDate.startsWith(selectedMonth)))
  const scheduled = monthScheduled(items, selectedMonth)
  const paidInMonth = monthPaid(items, selectedMonth)
  const scheduledRemaining = items.flatMap(item => item.payments).filter(payment => payment.scheduledDate.startsWith(selectedMonth) && payment.status === 'scheduled').reduce((sum, payment) => sum + payment.amount, 0)
  ...
  const totalMonthlyExpense = scheduled + recurringTotal
  const installmentMyTotal = items.reduce((sum, item) => sum + item.payments.filter(payment => payment.scheduledDate.startsWith(selectedMonth)).reduce((paymentSum, payment) => paymentSum + myShare(item, payment.amount), 0), 0)
  ...
  const endingInMonth = endingItems.length
  const splitMonthPayments = items.filter(item => item.splitPayment).flatMap(item => item.payments.map(payment => ({ item, payment }))).filter(row => row.payment.scheduledDate.startsWith(selectedMonth))
  const splitRecurring = recurringForMonth.filter(expense => expense.splitPayment)
  const splitMonthTotal = splitMonthPayments.reduce((sum, row) => sum + row.payment.amount, 0) + splitRecurring.reduce((sum, expense) => sum + expense.amount, 0)
  const splitMonthMine = splitMonthPayments.reduce((sum, row) => sum + myShare(row.item, row.payment.amount), 0) + splitRecurring.reduce((sum, expense) => sum + splitShares(expense.amount, expense.splitParticipants)[0].amount, 0)
  ...
  function updateSplitName(index: number, value: string) {
    ...
    setRecurringExpenses(prev => prev.map(expense => expense.splitPayment && expense.splitParticipants.length > index ? { ...expense, splitParticipants: expense.splitParticipants.map((current, i) => i === index ? value : current), updatedAt: new Date().toISOString() } : expense))
...
function HomeView({ monthItems, activeCount, selectedMonth, monthOptions, setSelectedMonth, scheduled, paid, scheduledRemaining, recurringTotal, totalMonthlyExpense, installmentMyTotal, recurringMyTotal, personalMonthlyExpense, sharedReimbursement, endingInMonth, remaining, splitTotal, splitMine, onOpen, onAdd, onOpenSplit, onOpenEnding }: { monthItems: InstallmentItem[]; activeCount: number; selectedMonth: string; monthOptions: string[]; setSelectedMonth: (value: string) => void; scheduled: number; paid: number; scheduledRemaining: number; recurringTotal: number; totalMonthlyExpense: number; installmentMyTotal: number; recurringMyTotal: number; personalMonthlyExpense: number; sharedReimbursement: number; endingInMonth: number; remaining: number; splitTotal: number; splitMine: number; onOpen: (item: InstallmentItem) => void; onAdd: () => void; onOpenSplit: () => void; onOpenEnding: () => void }) {
  ...
  return <>
    ...
    <section className="px-5">
      ...
      <div className="mt-3 rounded-[24px] bg-white p-5 dark:bg-[#211f29]"><p className="text-xs font-bold text-[#0284c7]">할부 납부 금액</p><p className="mt-1 text-2xl font-extrabold">{formatWon(scheduled)}</p><div className="mt-4 grid grid-cols-2 gap-2 text-sm"><div className="rounded-xl bg-[#f8f7fb] p-3 dark:bg-[#15141b]"><p className="text-xs text-gray-400">납부 완료</p><b>{formatWon(paid)}</b></div><div className="rounded-xl bg-sky-50 p-3 dark:bg-sky-500/10"><p className="text-xs text-gray-400">납부 예정</p><b>{formatWon(scheduledRemaining)}</b></div></div></div>
      <div className="mt-3 grid grid-cols-2 gap-3"><Summary icon={<Database size={19}/>} label="고정지출" value={formatWon(recurringTotal)}/><Summary icon={<CalendarDays size={19}/>} label={isPast ? '종료된 할부' : '종료 예정 할부'} value={`${endingInMonth}건`} onClick={onOpenEnding}/></div>
      <button onClick={onOpenSplit} className="mt-3 w-full rounded-[22px] border border-sky-100 bg-sky-50 p-4 text-left dark:border-sky-500/10 dark:bg-sky-500/10"><div className="flex items-center justify-between"><div><p className="text-xs font-bold text-[#0284c7]">{monthNumber}월 나눠 내기 · 정산 보기</p><p className="mt-1 text-lg font-extrabold">내 부담 {formatWon(splitMine)}</p></div><div className="flex items-center gap-2 text-right"><div><p className="text-xs text-gray-400">전체 결제액</p><p className="mt-1 text-sm font-bold">{formatWon(splitTotal)}</p></div><ChevronRight size={18} className="text-sky-400"/></div></div></button>
      <div className="mt-3 grid grid-cols-2 gap-3"><Summary icon={<WalletCards size={19}/>} label="전체 남은 금액" value={formatWon(remaining)}/><Summary icon={<CreditCard size={19}/>} label="진행 중인 할부" value={`${activeCount}건`}/></div>
    </section>
    <section className="mt-7"><div className="mb-3 flex items-center justify-between px-5"><h2 className="text-lg font-extrabold">{monthNumber}월 할부 내역</h2><span className="text-xs font-semibold text-gray-400">결제일 순</span></div><div className="space-y-3 px-5">{monthItems.length ? [...monthItems].sort((a, b) => (a.payments.find(payment => payment.scheduledDate.startsWith(selectedMonth))?.scheduledDate || '').localeCompare(b.payments.find(payment => payment.scheduledDate.startsWith(selectedMonth))?.scheduledDate || '')).map(item => <ItemCard key={item.id} item={item} paymentMonth={selectedMonth} onClick={() => onOpen(item)}/>) : <Empty onAdd={onAdd} message="선택한 월의 할부 내역이 없어요"/>}</div></section>
  </>
}
...
function ItemCard({ item, onClick, paymentMonth }: { item: InstallmentItem; onClick: () => void; paymentMonth?: string }) {
...
function StatsView({ items, recurringExpenses, remaining, selectedMonth, monthOptions, setSelectedMonth }: { items: InstallmentItem[]; recurringExpenses: RecurringExpense[]; remaining: number; selectedMonth: string; monthOptions: string[]; setSelectedMonth: (value: string) => void }) {
  ...
  return <><Header eyebrow={`${formatMonthKey(selectedMonth)} 소비 흐름`} title="지출 통계"/><MonthSelector value={selectedMonth} options={monthOptions} onChange={setSelectedMonth}/><div className="space-y-4 px-5"><div className="grid grid-cols-2 gap-3"><Summary icon={<WalletCards size={19}/>} label="전체 잔여 금액" value={formatWon(remaining)}/><Summary icon={<CreditCard size={19}/>} label="할부 납부 금액" value={formatWon(installmentTotal)}/></div><div className="grid grid-cols-2 gap-3"><Summary icon={<Database size={19}/>} label="고정지출" value={formatWon(recurringTotal)}/><Summary icon={<CreditCard size={19}/>} label="전체 결제액" value={formatWon(combinedTotal)}/></div><div className="grid grid-cols-2 gap-3"><Summary icon={<WalletCards size={19}/>} label="내 실질 부담액" value={formatWon(personalCombined)}/><Summary icon={<CreditCard size={19}/>} label="나눠 받을 금액" value={formatWon(reimbursement)}/></div><div className="rounded-[24px] bg-white p-5 dark:bg-[#211f29]"><h2 className="font-extrabold">할부 vs 고정지출</h2><p className="mt-1 text-xs text-gray-400">{Number(selectedMonth.slice(5, 7))}월 지출 비율</p><div className="mt-5 flex h-3 overflow-hidden rounded-full bg-gray-100 dark:bg-white/10"><div className="bg-[#0284c7]" style={{ width: `${combinedTotal ? installmentTotal / combinedTotal * 100 : 0}%` }}/><div className="bg-[#7dd3fc]" style={{ width: `${combinedTotal ? recurringTotal / combinedTotal * 100 : 0}%` }}/></div><div className="mt-3 flex justify-between text-xs"><span>할부 <b>{formatWon(installmentTotal)}</b></span><span>고정지출 <b>{formatWon(recurringTotal)}</b></span></div></div><div className="rounded-[24px] bg-white p-5 dark:bg-[#211f29]"><h2 className="font-extrabold">카테고리별 고정지출</h2><div className="mt-4 space-y-4">{recurringCategory.map(([name, value]) => <div key={name}><div className="mb-1.5 flex justify-between text-sm"><span>{name}</span><b>{formatWon(value)}</b></div><div className="h-2 rounded-full bg-gray-100 dark:bg-white/10"><div className="h-full rounded-full bg-[#38bdf8]" style={{ width: `${recurringTotal ? value / recurringTotal * 100 : 0}%` }}/></div></div>)}{!recurringCategory.length && <p className="text-sm text-gray-400">선택한 월의 고정지출이 없어요.</p>}</div></div><div className="rounded-[24px] bg-white p-5 dark:bg-[#211f29]"><p className="text-sm text-gray-400">전체 남은 할부 금액</p><p className="mt-1 text-2xl font-extrabold">{formatWon(remaining)}</p><div className="mt-6 h-44"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data}><defs><linearGradient id="fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#0284c7" stopOpacity={.35}/><stop offset="100%" stopColor="#0284c7" stopOpacity={0}/></linearGradient></defs><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#999' }}/><Tooltip formatter={value => formatWon(Number(value))}/><Area type="monotone" dataKey="amount" stroke="#0284c7" strokeWidth={3} fill="url(#fill)"/></AreaChart></ResponsiveContainer></div></div><div className="rounded-[24px] bg-white p-5 dark:bg-[#211f29]"><h2 className="font-extrabold">카테고리별 잔액</h2><div className="mt-4 space-y-4">{category.map(([name, value]) => <div key={name}><div className="mb-1.5 flex justify-between text-sm"><span>{name}</span><b>{formatWon(value)}</b></div><div className="h-2 rounded-full bg-gray-100 dark:bg-white/10"><div className="h-full rounded-full bg-[#0ea5e9]" style={{ width: `${remaining ? value / remaining * 100 : 0}%` }}/></div></div>)}{!category.length && <p className="text-sm text-gray-400">표시할 데이터가 없어요.</p>}</div></div></div></>
}
